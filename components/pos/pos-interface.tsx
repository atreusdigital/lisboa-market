    ...
    <div className="flex flex-col gap-4 h-[calc(100vh-8rem)]">
      ...
      {profile.role === 'director' && (
        <div className="flex items-center gap-3 p-3 rounded-lg border border-border bg-neutral-50">
          <span className="text-xs text-muted-foreground font-medium shrink-0">Vendiendo en:</span>
          ...
          {!selectedBranchId && (
            <span className="text-xs text-amber-600">Seleccioná una sucursal para ver productos</span>
          )}
        </div>
      )}
      ...
      <div className="flex gap-5 flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="relative">
            ...
            <Input
              placeholder="Buscar producto, categoría o código de barras..."
              value={search}
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        inCart ? 'border-2 ring-1' : 'border-border'
                      )}
        ...
        {/* Carrito */}
        <Card className="w-72 xl:w-80 flex flex-col border-border bg-white shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center text-sm text-muted-foreground gap-2 py-8">
                ...
                <p className="text-xs">Seleccioná productos de la izquierda</p>
              </div>
            ) : (
              cart.map((item) => (
                <div key={item.product.id} className="flex items-center gap-2 p-2 rounded-lg border border-border bg-neutral-50">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => updateQty(item.product.id, -1)} className="w-5 h-5 rounded flex items-center justify-center hover:bg-neutral-200 transition-colors">
                    ...
                    <button onClick={() => removeFromCart(item.product.id)} className="w-5 h-5 rounded flex items-center justify-center hover:bg-red-100 text-red-500 transition-colors ml-0.5">
