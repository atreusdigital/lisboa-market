...
import type { Stock, Profile, CartItem } from '@/types'
...
export function POSInterface({ stockItems, profile }: Props) {
  ...
  const filteredStock = useMemo(() => {
    if (!search) return stockItems.slice(0, 30)
  ...
  async function handleCheckout() {
    ...
    if (!profile.branch_id) {
      toast.error('No tenés una sucursal asignada')
      return
    }
    setLoading(true)
    ...
    try {
      ...
      for (const cartItem of cart) {
        const stockItem = stockItems.find(
          (s) => s.product?.id === cartItem.product.id && s.branch_id === profile.branch_id
        )
        if (stockItem) {
          await supabase
            .from('stock')
              ...
              <div key={item.product.id} className="flex items-center gap-2 p-2 rounded-lg border border-border bg-neutral-50">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className="w-5 h-5 rounded flex items-center justify-center hover:bg-neutral-200 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="w-5 h-5 rounded flex items-center justify-center hover:bg-red-100 text-red-500 transition-colors ml-0.5"
                  >
